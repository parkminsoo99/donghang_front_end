...
export function userError({name, userNickName, password}:userDetailProps): ErrorProps {
  ...
  if (isNil(name) || name.length === 0) {
    ...
    userError.errorDescription = '비밀번호는 대문자, 소문자, 숫자, 특수문자 포함 및 최소 10자 이상 입니다.';
  }
  return userError;
}
...
export function passwordError(password:passwordEmailProps): ErrorProps {
  ...
  if (isNil(password) || password.length == 0) {
    ...
    passwordError.errorDescription = '비밀번호는 대문자, 소문자, 숫자, 특수문자 포함 및 최소 10자 이상 입니다.';
  }
